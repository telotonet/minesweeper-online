...
class GameConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data):
        ...
        if message_type == 'open' and self.user_key in self.player_list and self.game_obj.lives > 0:
            x = text_data_json.get('x')
            y = text_data_json.get('y')
            opened_cells = []
            await self.open_handle_message(x, y, opened_cells)

        if message_type == 'message':
            message = text_data_json.get('message')
            await self.chat_handle_message(message, self.user_key)    
    ...
    async def open_handle_message(self, x, y, opened_cells):
        if self.game_obj.game_state[x][y] in ['q', 'c', 'f']:
            if self.game_obj.board[x][y] == '0':
                ...
                if self.game_obj.board[x][y] == 'M' and self.game_obj.lives > 0:
                    ...
                    if self.game_obj.lives == 0:
                        self.game_obj.status = '2'
                        await self.channel_layer.group_send(
                            self.game_group_name,
                            {
    ...
    async def start_handle_message(self):
        random_zero_cells = choice(self.find_all_zero_cells(self.game_obj.board))
        await self.open_handle_message(random_zero_cells[0], random_zero_cells[1], [])
        await self.channel_layer.group_send(
            self.game_group_name,
            {
    ...
    async def game_handle_message(self, message):
        await self.channel_layer.group_send(
            self.game_group_name,
            {
                'type': 'send_game_message',
    ...
    def find_all_zero_cells(self, board):
        zero_cells = []
